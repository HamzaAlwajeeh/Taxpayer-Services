...
class NotificationService {
  ...
  static const String _channelName = 'إشعارات عامة';
  static const String _channelDesc =
      'التنبيهات والتحديثات العامة لمصلحة الضرائب.';
  ...
  static Future<void> scheduleTaxRemindersJanToApr() async {
    try {
      ...
      for (int i = 0; i < dates.length; i++) {
        var scheduled = dates[i];

        if (scheduled.isBefore(now)) {
          // If the date has passed this year, schedule it for the next year
